// laba3.1.cpp : Определяет точку входа для приложения.
//
...
public:
    ...
    Vector2D operator*(const int k) const {
        return Vector2D(x *k, y * k);
    }
    friend void DrawEllipse(HDC, const Vector2D&, const int);

};

void DrawEllipse(HDC hdc, const Vector2D& center, const int radius) {
    ...
    case WM_PAINT:
        {
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    Vector2D offset(20 * i, 20 * j);
                    centers.push_back(v0 + offset);
                }
            }
            int i = 0;
            while (i < num_circles) {
